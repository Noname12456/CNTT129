...
namespace CNTT129.Models
{
    public class HOCKY
    {
        ...
        public int updateDiem(string idhk, string diem, string diemmd)
        {
            ...
            con.Open();
            SqlCommand cmd10 = new SqlCommand("update HOATDONGTHEONGAY set HOATDONGTHEONGAY.TRANGTHAI = 8 from HOATDONG where HOATDONG.IDHD = HOATDONGTHEONGAY.IDHD and HOATDONG.ID_HK = " + idhk + " and HOATDONGTHEONGAY.TRANGTHAI = 6", con);// Đã chốt
            ...
            con.Open();
            SqlCommand cmd11 = new SqlCommand("INSERT INTO KETQUA(IDSV,IDHK,DIEMMD,DIEM,LOAI) select ID_SV," + (int.Parse(idhk) + 1) + "," + diem + ",0,'B' from SINHVIEN where disabled = 0 and ID_SV NOT IN (SELECT IDSV FROM KETQUA WHERE IDHK = " + idhk + ") ", con);
        ...
        public int updateDiem2(string idhk, string ngaybd, string ngaykt)
        {
            ...
            con.Open();
            SqlCommand cmd2 = new SqlCommand("update KETQUA set DIEM += HOATDONG.DIEMRL from PHAN_HOI,HOATDONGTHEONGAY,HOATDONG where KETQUA.IDSV = PHAN_HOI.IDSV and HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD and PHAN_HOI.IDBUOI = HOATDONGTHEONGAY.IDBUOI and PHAN_HOI.IDHK = " + idhk + " and PHAN_HOI.TRANGTHAI = 1 and HOATDONGTHEONGAY.TRANGTHAI = 6 " + sql, con);
            cmd2.CommandType = CommandType.Text;
            dr = cmd2.ExecuteNonQuery();
            con.Close();
            ...
            con.Open();
            SqlCommand cmd10 = new SqlCommand("update HOATDONGTHEONGAY set HOATDONGTHEONGAY.TRANGTHAI = 8 from HOATDONG where HOATDONG.IDHD = HOATDONGTHEONGAY.IDHD and HOATDONG.ID_HK = " + idhk + " and HOATDONGTHEONGAY.TRANGTHAI = 6 " + sql, con);// Đã chốt
            cmd10.CommandType = CommandType.Text;
            dr = cmd10.ExecuteNonQuery();
            con.Close();
