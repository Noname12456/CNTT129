...
namespace CNTT129.Models
{
    public class SV_CHUCVU
    {
        ...
        public int updateDiem()
        {
            ...
            con.Open();
            string sql = "";
            sql = "update KETQUA set DIEM += VAI_TRO_SV.DIEM from SV_CHUCVU,VAI_TRO_SV where KETQUA.IDSV = SV_CHUCVU.ID_SV  and SV_CHUCVU.ID_VAI_TRO_SV = VAI_TRO_SV.ID_VAI_TRO_SV and SV_CHUCVU.disabled = 0";
            if (sql != "")
            {
                SqlCommand cmd = new SqlCommand(sql, con);
                cmd.CommandType = CommandType.Text;
