...
namespace CNTT129.Models
{
    public class HOATDONG
    {
        ...
        public List<HOATDONG> layDanhHoatDongChuaChotSV(string IDSV, string idhk)
        {
            List<HOATDONG> listHD = new List<HOATDONG>();
            SqlConnection con = new SqlConnection(conf);
            SqlCommand cmd = new SqlCommand("SELECT HOATDONG.MAHD,HOATDONG.IDHD,HOATDONG.TIEUDE,HOATDONGTHEONGAY.IDBUOI,HOATDONGTHEONGAY.NGAYBATDAUDIEMDANH FROM HOATDONGTHEONGAY  LEFT JOIN DANGKY ON HOATDONGTHEONGAY.IDBUOI = DANGKY.IDBUOI   INNER JOIN HOATDONG ON HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD WHERE DANGKY.IDSV = " + IDSV + "   AND HOATDONGTHEONGAY.TRANGTHAI = 3  and HOATDONGTHEONGAY.disabled = 0  and DANGKY.TRANGTHAI NOT IN (1,2) and HOATDONG.ID_HK = " + idhk + "   and HOATDONGTHEONGAY.IDBUOI  not in  ( SELECT HOATDONGTHEONGAY.IDBUOI FROM HOATDONGTHEONGAY  INNER JOIN HOATDONG ON HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD INNER JOIN PHAN_HOI ON PHAN_HOI.IDBUOI = HOATDONGTHEONGAY.IDBUOI    WHERE PHAN_HOI.IDSV = " + IDSV + " AND HOATDONGTHEONGAY.TRANGTHAI = 3 and HOATDONGTHEONGAY.disabled = 0  and HOATDONG.ID_HK = " + idhk + ") and (DATEDIFF(day, HOATDONGTHEONGAY.NGAYBATDAUDIEMDANH,GETDATE()) < 7  or DATEDIFF(day, HOATDONGTHEONGAY.NGAYBATDAUDIEMDANH,GETDATE()) = 7 )  UNION ALL	  SELECT HOATDONG.MAHD,HOATDONG.IDHD,HOATDONG.TIEUDE,HOATDONGTHEONGAY.IDBUOI,HOATDONGTHEONGAY.NGAYBATDAUDIEMDANH FROM HOATDONGTHEONGAY INNER JOIN HOATDONG ON HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD     where  HOATDONGTHEONGAY.disabled = 0 and HOATDONGTHEONGAY.IDBUOI  not in (SELECT HOATDONGTHEONGAY.IDBUOI FROM HOATDONGTHEONGAY      LEFT JOIN DANGKY ON HOATDONGTHEONGAY.IDBUOI = DANGKY.IDBUOI     INNER JOIN HOATDONG ON HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD      WHERE DANGKY.IDSV = " + IDSV + " AND HOATDONGTHEONGAY.TRANGTHAI = 3   and HOATDONGTHEONGAY.disabled = 0 and DANGKY.TRANGTHAI NOT IN (1,2)   and HOATDONG.ID_HK = " + idhk + ")   AND HOATDONGTHEONGAY.TRANGTHAI = 3   and HOATDONGTHEONGAY.disabled = 0 and HOATDONG.ID_HK = " + idhk + " and HOATDONGTHEONGAY.IDBUOI  NOT IN (SELECT HOATDONGTHEONGAY.IDBUOI FROM HOATDONGTHEONGAY  INNER JOIN HOATDONG ON HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD   INNER JOIN PHAN_HOI ON PHAN_HOI.IDBUOI = HOATDONGTHEONGAY.IDBUOI   WHERE PHAN_HOI.IDSV = " + IDSV + " AND HOATDONGTHEONGAY.TRANGTHAI = 3 and HOATDONGTHEONGAY.disabled = 0  and HOATDONG.ID_HK = " + idhk + ")   and HOATDONGTHEONGAY.IDBUOI  not in (SELECT HOATDONGTHEONGAY.IDBUOI FROM HOATDONGTHEONGAY       LEFT JOIN DIEMDANH ON HOATDONGTHEONGAY.IDBUOI = DIEMDANH.IDBUOI   INNER JOIN HOATDONG ON HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD    WHERE DIEMDANH.IDSV = " + IDSV + " AND HOATDONGTHEONGAY.TRANGTHAI = 3  and HOATDONGTHEONGAY.disabled = 0  and HOATDONG.ID_HK = " + idhk + ") and (DATEDIFF(day, HOATDONGTHEONGAY.NGAYBATDAUDIEMDANH,GETDATE()) < 7  or DATEDIFF(day, HOATDONGTHEONGAY.NGAYBATDAUDIEMDANH,GETDATE()) = 7 ) ", con);
            cmd.CommandType = CommandType.Text;
            con.Open();
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
        ...
        public int demSL(string idhk, string ngaybd, string ngaykt)
        {
            ...
            SqlConnection con = new SqlConnection(conf);
            SqlCommand cmd2 = new SqlCommand("select count(*) from HOATDONG INNER JOIN HOATDONGTHEONGAY ON HOATDONGTHEONGAY.IDHD = HOATDONG.IDHD where HOATDONGTHEONGAY.trangthai NOT IN (6,7,8,1) and HOATDONG.ID_HK = " + idhk + " and HOATDONGTHEONGAY.disabled = 0 " + sql, con);
            cmd2.CommandType = CommandType.Text;
            con.Open();
            Object kq = cmd2.ExecuteScalar();
